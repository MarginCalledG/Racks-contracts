...
contract Racks is ReentrancyGuard {
    ...
    function ratePerDayBpsFor(uint8 pos) external view returns (uint256) {
    ...
    address public pair;
    uint256 public pairIndex;                       // kept for tooling: index at last pool melt
    ...
    function meltPool() public {
        ...
        if (pairLastMelt != 0 && dtPool > 0) {
            ...
            // LP melts at HALF the unlocked rate (factor 0.5)
            uint256 melt = bal - RayMath.rmul(bal, RayMath.rpow(perSecFactorFor(P_LP), dtPool));
            if (melt > 0) {
