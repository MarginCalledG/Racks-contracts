...
contract AuditVault is Test {
    ...
    function testHarvestSpamCannotHurtOwner() public {
        ...
        assertApproxEqRel(v.pot(), 20_000 ether, 0.01e18, "pot must only get the in-lock bleed");
    }
    ...
    function testExpiredMeltIsBurned() public {
        ...
        vm.warp(block.timestamp + 14 days + 5 days);
        uint256 supplyBefore = k.totalSupply();
        uint256 claimBefore = 1_000_000 ether; // no bleed on tier 2
        v.harvest(alice, 2);
        uint256 melted = claimBefore - v.claimOf(alice, 2);
        assertApproxEqAbs(supplyBefore - k.totalSupply(), melted, 1e9, "burn != melt");
        assertEq(v.pot(), 0, "tier-2 melt must not touch the pot");
    }
    ...
    function testRelockAfterExpiryAppliesMelt() public {
        ...
        assertGt(principal, 1_000_000 ether * 78 / 100);
    }
